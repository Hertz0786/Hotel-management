...
namespace Hotel_Management
{
    public partial class RegisterForm : Form
    {
        ...
        private void register_showpass_CheckedChanged(object sender, EventArgs e)
        {
            ...
            register_cfpassword.PasswordChar = register_showPass.Checked ? '\0' : '*';

        }

        private void register_signupbtn_Click(object sender, EventArgs e)
        {
            if(register_username.Text == "" || register_password.Text == "" || register_cfpassword.Text == "")
            {
            ...
            else
            {
                using(SqlConnection connect = new SqlConnection(conn))
                {
                    ...
                    using (SqlCommand checkU = new SqlCommand(checkUsern, connect))
                    {
                        ...
                        else if(register_password.Text.Length < 5)
                        {
                            MessageBox.Show("Invalid password", "Error Message",MessageBoxButtons.OK,MessageBoxIcon.Error);
                        }
                        else if(register_password.Text != register_cfpassword.Text)
                        {
