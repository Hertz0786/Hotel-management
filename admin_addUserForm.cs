...
namespace Hotel_Management
{
    public partial class admin_addUserForm : UserControl
    {
            ...
            else
            {
                using (SqlConnection connect = new SqlConnection(conn))
                {
                    ...
                    using (SqlCommand checkU = new SqlCommand(checkUsern, connect))
                    {
                        ...
                        else if (addUser_password.Text.Length < 5)
                        {
                        ...
                        else
                        {
                            ...
                            using (SqlCommand cmd = new SqlCommand(insertData, connect))
                            {
                                ...
                                cmd.ExecuteNonQuery();
                                clearFields();
                                MessageBox.Show("Add succesfully", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Error);
                            }
                        }
        ...
        private void admin_addUserForm_Load(object sender, EventArgs e)
        {
        ...
        private void addUser_updateBtn_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (MessageBox.Show("Are you sure want to update ID: " + getID + "?", "Confirmation Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    {
                        connect.Open();
                        string updateData = "UPDATE users SET password - @pass, rol - @role, status - @status WHERE username - @usern";
                        ...
                        using (SqlCommand cmd = new SqlCommand(updateData, connect))
                        {
                            ...
                            MessageBox.Show("Updated succsesfully", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ...
                if (MessageBox.Show("Are you sure want to Delete ID: " + getID + "?", "Confirmation Message", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    using (SqlConnection connect = new SqlConnection(conn))
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(updateData, connect))
                        {
                            ...
                            MessageBox.Show("Delete succsesfully", "Information Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private void addUser_clearname_Click(object sender, EventArgs e)
        {
            clearFields();
        }
    }
