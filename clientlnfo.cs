...
{
    public partial class clientlnfo : Form
    {
        ...
        {
            if(MessageBox.Show("Are you sure want to book now? ", "Confirmation Message",MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.Yes)
            {
                if (client_fullName.Text == "" || client_gender.SelectedIndex == -1 || client_address.Text == "" || client_email.Text == "" || client_contact.Text == "" || hotelData.roomID == "")
                {
                ...
                else
                {
                    using (SqlConnection connect = new SqlConnection(conn))
                    {
                        ...
                        string insertData = "INSERT INTO customer" +
                            ...
                            ", date_from, date_to, date_book) " + "VALUES(@bookID, @fullname, @email, @contact, @gender, @address, @roomID, @price, @statusP, @status, @dateForm, @dateTo, @dateBook)";
